        ...
        <div class="container">
            ...
            <div class="timer">
                <svg class="timer-svg">
                    ...
                    <circle id="timer-circle" r="35" cx="40" cy="40" fill="transparent" stroke="var(--primary-color)" stroke-width="8" stroke-linecap="round" stroke-dasharray="219.9" stroke-dashoffset="219.9"></circle>
                </svg>
        ...
        if (result.errors.length === 0 && (data.code === 100 || data.code === 101)) {
            ...
            if (renewalIdentifier && renewalIdentifier !== '') {
                ...
                if(chat_id && chat_id !== 'none') { // from bot
                    newRow.telegramUsername = name; // Storing firstname + lastname
                    newRow.telegramId = email; // Storing telegram ID
                ...
                await renewSheet.addRow(newRow);
            ...
            if (chat_id && chat_id !== 'none') {
                await bot.sendMessage(chat_id, `✅ پرداخت موفق!\n🔗 لینک شما:\n\`${userLink}\`\n🔢 شماره پیگیری: \`${trackingId}\``, { 
                    parse_mode: 'Markdown',
                    reply_markup: { inline_keyboard: [[{ text: '👁️ مشاهده اشتراک', url: userLink }]] }
                });
        ...
        if (sheet) {
            ...
            rows.forEach(row => {
                ...
                if ((email && rowEmail === email) || (phone && rowPhone === phone)) {
                    if(row.get('status') === 'used') { // Only show used links
