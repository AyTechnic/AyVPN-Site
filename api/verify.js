...
// کلید خصوصی نیاز به فرمت‌بندی دارد تا کاراکترهای خط جدید به درستی خوانده شوند
const GOOGLE_PRIVATE_KEY = process.env.GOOGLE_PRIVATE_KEY.replace(/\\n/g, '\n'); 

// --- نقشه مبالغ جدید به نام شیت‌ها (بر اساس قیمت‌های جدید شما) ---
...
export default async function handler(req, res) {
    ...
    let amountForVerification;

    try {
        // برای امنیت و دقت، مبلغ را مستقیماً از زرین‌پال استعلام می‌کنیم
        const unverifiedResponse = await axios.post('https://api.zarinpal.com/pg/v4/payment/unverified.json', {
            merchant_id: ZARINPAL_MERCHANT_ID,
            authority: Authority,
        });

        // در API جدید، زرین‌پال مبلغ را در یک آرایه برمی‌گرداند
        if (unverifiedResponse.data.data && unverifiedResponse.data.data.transactions.length > 0) {
            amountForVerification = unverifiedResponse.data.data.transactions[0].amount;
        } else {
            throw new Error('تراکنش نامعتبری برای این کد رهگیری یافت نشد.');
        }
        ...
        const verificationResponse = await axios.post('https://api.zarinpal.com/pg/v4/payment/verify.json', {
            merchant_id: ZARINPAL_MERCHANT_ID,
            authority: Authority,
            amount: amountForVerification, 
        });

        const { data } = verificationResponse.data;

        if (data.code === 100 || data.code === 101) { // کد 101 یعنی تراکنش قبلا با موفقیت تایید شده
            // 2. پیدا کردن نام شیت از روی مبلغ
            const sheetName = planToSheetMap[amountForVerification.toString()];
            if (!sheetName) {
                throw new Error(`پلنی برای مبلغ ${amountForVerification} تومان یافت نشد.`);
            }

            // 3. اتصال به گوگل شیت و پیدا کردن لینک
            ...
            if (!sheet) {
                throw new Error(`شیت با نام "${sheetName}" یافت نشد.`);
            }
            ...
            // 4. تحویل لینک و آپدیت کردن وضعیت آن
            const userLink = availableLinkRow.get('link');
            availableLinkRow.set('status', 'used-' + new Date().toISOString()); // وضعیت را به همراه تاریخ ثبت می‌کنیم
            await availableLinkRow.save();

            // 5. نمایش لینک به کاربر
...
function generateResponseMessage(title, message, type = 'success', link = null) {
    ...
    return `
        <html lang="fa"><head><meta charset="UTF-8"><title>${title}</title><style>
            ...
            .icon { font-size: 3rem; color: ${color.text}; }
            h1 { color: #333; font-size: 1.5rem; margin-top: 20px; }
